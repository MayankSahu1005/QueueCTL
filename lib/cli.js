...
function enqueueFromJson(jsonStr) {
  ...
  // Convert ISO string to Unix timestamp (milliseconds) for scheduled_at
  const scheduledAt = job.scheduled_at ? new Date(job.scheduled_at).getTime() : new Date(now).getTime();
